...
const scrypt = (value: string, salt: string) =>
  new Promise<Buffer>((resolve, reject) =>
    scryptCb(value, salt, 64, (err, key) => (err ? reject(err) : resolve(key as Buffer))),
  );
...
export async function apiUser(allowed?: string[]) {
  const user = await getCurrentUser();
  if (!user) return null;
...
export async function apiPermission(module: PermissionModule, action: PermissionAction = "view") {
  const user = await getCurrentUser();
  if (!user) return null;
  return (await hasPermission(user.id, user.role, module, action)) ? user : null;
}
