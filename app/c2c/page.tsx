...
export default function C2cPage() {
  ...
  return <main className="page ops-premium-page">
    ...
    <section className="ops-section"><OpsSectionTitle title="Employee Recharge Performance" subtitle="C2C, SC and total recharge progress across the selected date range." icon="↗"/>
      <div className="ops-metrics-grid six"><OpsMetric tone="blue" label="C2C Target" value={money(totals.c2cT)} note="Selected range target"/><OpsMetric tone="green" label="C2C Achieved" value={money(totals.c2cA)} note="Stock lifting"/><OpsMetric tone="purple" label="C2C %" value={totals.c2cT?`${((totals.c2cA/totals.c2cT)*100).toFixed(1)}%`:"0%"} note="Achievement rate"/><OpsMetric tone="cyan" label="SC Achieved" value={money(totals.sc)} note="Included for fully covered months"/><OpsMetric tone="orange" label="Total Recharge" value={money(totals.trA)} note="C2C + SC"/><OpsMetric tone="rose" label="Monthly Transactions" value={totals.trx.toLocaleString()} note="Exact TRANSACTION_COUNT from source"/></div>
    </section>

    <OpsDataCard title="RSO Recharge Performance" subtitle="Target, achieved and progress by field employee." count={`${rows.length} employees`}>
      {rows.length?<OpsTable minWidth={1120}><thead><tr><th>Supervisor</th><th>Employee</th><th>Retailers</th><th>Transactions</th><th>C2C Target</th><th>C2C Achieved</th><th>C2C Progress</th><th>SC</th><th>Recharge Target</th><th>Recharge Achieved</th><th>Total Progress</th></tr></thead><tbody>{rows.map(r=><tr key={r.employeeId}><td><PersonCell name={r.supervisor} sub="Supervisor"/></td><td><PersonCell name={r.name} sub={r.employeeCode||r.rsoMsisdn}/></td><td><span className="ops-neutral-pill">{r.retailerCount}</span></td><td>{r.transactionCount}</td><td>{money(r.c2cTarget)}</td><td><strong className="ops-number green">{money(r.c2cAchieved)}</strong></td><td><ProgressCell value={r.c2cPercent}/></td><td><span className="ops-number cyan">{money(r.scAchieved)}</span></td><td>{money(r.totalRechargeTarget)}</td><td><strong className="ops-number blue">{money(r.totalRechargeAchieved)}</strong></td><td><ProgressCell value={r.totalRechargePercent}/></td></tr>)}</tbody></OpsTable>:<EmptyState title="No C2C performance yet" subtitle="Upload C2C data to populate employee performance."/>}
    </OpsDataCard>

    <section className="ops-section"><OpsSectionTitle title={`Date-wise C2C · ${new Date(date+"T00:00:00").toLocaleDateString()}`} subtitle="Retailers that received balance on the selected day." icon="▣" right={<label className="ops-inline-date"><span>VIEW DATE</span><input type="date" value={date} onChange={e=>{setDate(e.target.value);if(e.target.value)setMonth(e.target.value.slice(0,7))}}/></label>}/>
      <div className="ops-metrics-grid two"><OpsMetric tone="blue" label="Selected Day Amount" value={money(dayTotal)} note="Total C2C distributed"/><OpsMetric tone="purple" label="Retailers Receiving Balance" value={dailyRows.length.toLocaleString()} note="Active recipients"/></div>
    </section>

    <OpsDataCard title="Retailer C2C" subtitle="Retailer-level balance distribution for the selected date." count={`${dailyRows.length} retailers`}>
      {dailyRows.length?<OpsTable><thead><tr><th>Supervisor</th><th>Employee</th><th>Retailer</th><th>Amount</th></tr></thead><tbody>{dailyRows.map(r=><tr key={r.retailerCode}><td><PersonCell name={r.supervisor}/></td><td><PersonCell name={r.employee} sub={r.rsoMsisdn}/></td><td><b>{r.retailerCode}</b><small className="ops-subline">{r.retailerName}</small></td><td><strong className="ops-number blue">৳{money(r.amount)}</strong></td></tr>)}</tbody></OpsTable>:<EmptyState title="No C2C on this date" subtitle={`No C2C amount stored for ${date}.`}/>}
    </OpsDataCard>

    <OpsDataCard title="Recent C2C Imports" subtitle="Latest uploaded Stock Lifting reports." count={`${history.length} imports`}>
      {history.length?<OpsTable><thead><tr><th>Report End</th><th>File</th><th>Uploaded</th><th>Mapped / Total</th><th>Failed</th><th>Status</th></tr></thead><tbody>{history.map(h=><tr key={h.id}><td><b>{h.businessDate?new Date(h.businessDate).toLocaleDateString():"-"}</b></td><td>{h.fileName}</td><td>{new Date(h.uploadedAt).toLocaleString()}</td><td><strong className="ops-number blue">{h.successRows}/{h.totalRows}</strong></td><td>{h.failedRows}</td><td><StatusPill value={h.status}/></td></tr>)}</tbody></OpsTable>:<EmptyState title="No C2C imports yet" subtitle="Your recent C2C files will appear here." icon="⇧"/>}
    </OpsDataCard>
  </main>
}
