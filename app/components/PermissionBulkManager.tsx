...
export default function PermissionBulkManager({users}:{users:U[]}){
 ...
 return <section className="card permission-bulk">
  ...
  <div className="permission-bulk-tools">
   <div className="permission-bulk-tool"><div><strong>Apply preset</strong><span>Replace selected users' effective access.</span></div><select className="control" value={preset} onChange={e=>setPreset(e.target.value)}><option value="ROLE_DEFAULT">Role Default</option><option value="VIEW_ONLY">View Only</option><option value="DATA_OPERATOR">Data Operator</option><option value="FULL_NON_ADMIN">Full Role Access</option></select><button className="btn admin-primary" disabled={busy} onClick={applyPreset}>Apply</button></div>
