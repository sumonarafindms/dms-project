...
export function EmployeeDetailView({d,month,basePath,backHref,q="",from,to}:{d:any;month:string;basePath:string;backHref:string;q?:string;from?:string;to?:string}){
 ...
 const manager=basePath==="/manager",supervisor=basePath==="/supervisor";return <main className={`page ${manager?"manager-v5-page manager-rso-detail-v5":supervisor?"supervisor-v6-page supervisor-rso-detail-v6":""}`}><PageHead eyebrow="RSO Detail" title={d.employee.name} subtitle={`${d.employee.employeeCode||d.employee.rsoMsisdn} · ${d.employee.supervisor?.name||"Unassigned supervisor"}`} action={<Link href={backHref} className="btn btn-ghost">Back</Link>}/>
 ...
 <LinkedList title={`Retailers (${filtered.length})`} items={filtered.map((r:any)=>({href:`${basePath}/retailers/${r.id}?month=${month}${from?`&from=${from}`:""}${to?`&to=${to}`:""}`,name:r.retailerName||r.retailerCode,meta:`${r.retailerCode}${r.isBp?" · BP":""}${(r.simSeller||"").toUpperCase()==="Y"?" · SIM Seller":""}`,right:`GA ${r.ga}`,status:`${r.lso?"LSO ✓":"LSO pending"} · ৳${Math.round(r.c2cAmount).toLocaleString()}`}))}/>
 </main>
}
