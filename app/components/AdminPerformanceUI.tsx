...
export function PerfHead({title,subtitle,month,q,placeholder="Search",from,to}:{title:string;subtitle:string;month:string;q:string;placeholder?:string;from?:string;to?:string}){
 ...
 return <><section className="perf-v2-head"><div><div className="admin-kicker">PERFORMANCE INTELLIGENCE</div><h1>{title}</h1><p>{subtitle}</p></div><div className="perf-v2-period"><span>REPORTING PERIOD</span><strong>{start}</strong><i>→</i><strong>{end}</strong></div></section><form className="perf-filter date-perf-filter perf-v2-filter"><div className="perf-search"><span><Icon name="search"/></span><div><label>SEARCH</label><input name="q" defaultValue={q} placeholder={placeholder}/></div></div><label><span>FROM</span><input type="date" name="from" defaultValue={start}/></label><label><span>TO</span><input type="date" name="to" defaultValue={end}/></label><button className="btn admin-primary"><Icon name="search"/>Apply</button></form></>
}
