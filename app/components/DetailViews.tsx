...
export function RetailerDetailView({d,month,backHref}:{d:any;month:string;backHref:string}){
 ...
 return <main className="page retailer-profile-v2">
  ...
  <section className="retailer-v2-kpis">
   <ProfileKpi label="GA Total" value={d.ga.length} sub={`${d.ga150} × 150 · ${d.ga300} × 300`} tone="blue"/>
  ...
  <section className="retailer-status-grid">
   <article className={`retailer-status-card ${d.ssoComplete?"done":"pending"}`}><span>SSO STATUS</span><strong>{simSeller?(d.ssoComplete?"Completed":"Pending"):"Not applicable"}</strong><small>{simSeller?`${d.ga.length} GA in selected range`:"Retailer is not marked SIM seller"}</small></article>
  ...
  <section className="retailer-v2-grid">
   <div className="retailer-v2-panel"><div className="retailer-v2-panel-head"><div><span>ACTIVATIONS</span><h2>Recent GA</h2></div><b>{d.ga.length}</b></div><div className="retailer-activity-v2">{d.ga.length?d.ga.slice(0,20).map((x:any)=><div className="retailer-activity-row" key={x.simNo}><span className="activity-type sim">SIM</span><div><strong>{x.simNo}</strong><small>{x.activationDate.toISOString().slice(0,10)}{x.activationTime?` · ${x.activationTime}`:""}</small></div><div><strong>৳{Number(x.sellingPrice)}</strong><small>{Number(x.sellingPrice)===170?"150":"300"} pack</small></div></div>):<div className="empty">No GA in this period.</div>}</div></div>
