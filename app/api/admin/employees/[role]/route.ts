 ...
 try{
  ...
  if(r==="supervisors"){
   ...
   await prisma.$transaction(async tx=>{
    ...
    if(supervisor.user){
      const udata:any={displayName:name,active};if(mobile)udata.mobileNumber=mobile;if(pin){if(pin.length<4)throw new Error("PIN must contain at least 4 characters.");udata.credentialHash=await hashCredential(pin)}await tx.user.update({where:{id:supervisor.user.id},data:udata});
  ...
  if(r==="rsos"){
   ...
   await prisma.$transaction(async tx=>{
    ...
    if(employee.user){
      const udata:any={displayName:name,active};if(mobile)udata.mobileNumber=mobile;if(pin){if(pin.length<4)throw new Error("PIN must contain at least 4 characters.");udata.credentialHash=await hashCredential(pin)}await tx.user.update({where:{id:employee.user.id},data:udata});
  ...
  if(r==="bps"){
   ...
   await prisma.$transaction(async tx=>{
    await tx.bpAssignment.update({where:{id},data:{gaTarget,active,...(!active&&a.active?{endDate:new Date()}: {})}});
    if(a.retailer.bpUser){
      const udata:any={displayName:name,active};if(mobile)udata.mobileNumber=mobile;if(pin){if(pin.length<4)throw new Error("PIN must contain at least 4 characters.");udata.credentialHash=await hashCredential(pin)}await tx.user.update({where:{id:a.retailer.bpUser.id},data:udata});
