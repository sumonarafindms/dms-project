  ...
  await prisma.$transaction(async (tx) => {
    for (const row of rows) {
      ...
      await tx.userPermission.upsert({
        ...
        create: { userId, module: moduleKey, canView, canAdd, canEdit, canUpdate },
      });
    }
  });
}
...
export async function POST(req: Request) {
  ...
  if (mode === "preset") {
    ...
    await audit(me, "BULK_PERMISSION_PRESET", "permissions", {
      detail: `Applied ${preset} to ${users.length} user(s)`,
      metadata: { preset, count: users.length },
    });
    return NextResponse.json({ ok: true, updated: users.length });
  }
  if (mode === "copy") {
    const sourceId = String(b.sourceId || ""),
      targetIds = Array.isArray(b.targetIds) ? b.targetIds.map(String) : [];
    const [source, targets] = await Promise.all([
      prisma.user.findUnique({ where: { id: sourceId }, include: { permissions: true } }),
      prisma.user.findMany({
        where: { id: { in: targetIds }, role: { not: "ADMIN" } },
        select: { id: true, role: true },
      }),
    ]);
    if (!source || ["ADMIN", "IT"].includes(source.role))
    ...
    for (const target of targets) {
      await saveRows(target.id, sourceRows);
    }
    await audit(me, "COPY_PERMISSIONS", "permissions", {
      targetType: "User",
      targetId: source.id,
      targetName: source.displayName,
      detail: `Copied permissions to ${targets.length} user(s)`,
