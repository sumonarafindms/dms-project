...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params,
    user = await prisma.user.findUnique({ where: { id }, include: { permissions: true } });
  if (!user) return NextResponse.json({ error: "User not found" }, { status: 404 });
  const custom = new Map(user.permissions.map((p) => [p.module, p]));
  return NextResponse.json({
    user: { id: user.id, name: user.displayName, role: user.role, mobile: user.mobileNumber },
    modules: permissionModules.map((m) => {
      const p = custom.get(m.key),
        d = roleDefaults[user.role]?.[m.key] || { view: false, add: false, edit: false, update: false };
      return {
        ...m,
        view: p?.canView ?? d.view,
        add: p?.canAdd ?? d.add,
        edit: p?.canEdit ?? d.edit,
        update: p?.canUpdate ?? d.update,
      };
    }),
  });
}
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await prisma.$transaction(async (tx) => {
    for (const row of rows) {
      ...
      const canView = Boolean(row.view),
        canAdd = canView && Boolean(row.add),
        canEdit = canView && Boolean(row.edit),
        canUpdate = canView && Boolean(row.update);
      await tx.userPermission.upsert({
        where: { userId_module: { userId: id, module: moduleKey } },
        update: { canView, canAdd, canEdit, canUpdate },
        create: { userId: id, module: moduleKey, canView, canAdd, canEdit, canUpdate },
      });
    }
  });
  await audit(me, "UPDATE_PERMISSIONS", "permissions", {
    targetType: "User",
    targetId: user.id,
    targetName: user.displayName,
    detail: "Saved custom module permissions",
  });
