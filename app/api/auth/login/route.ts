...
const throttleKey=(identifier:string,admin:boolean,client:string)=>{
  ...
  return createHash("sha256").update(`${admin?"admin":"field"}:${normalized}:${client}`).digest("hex");
};

export async function POST(req:Request){
  try{
    const body=await req.json();
    const identifier=String(body.identifier||"").trim(),credential=String(body.credential||""),admin=!!body.admin;
    if(!identifier||!credential)return NextResponse.json({error:"Mobile/username and PIN/password are required."},{status:400});

    const key=throttleKey(identifier,admin,clientHint(req)),now=new Date();
    await prisma.loginThrottle.deleteMany({where:{updatedAt:{lt:new Date(Date.now()-24*60*60*1000)}}}).catch(()=>{});
  ...
  }catch(error){console.error(error);const e=apiError(error,"Unable to sign in right now.");return NextResponse.json({error:e.error},{status:e.status})}
}
