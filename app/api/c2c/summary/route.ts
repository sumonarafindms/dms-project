...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const rows = employees.map((employee) => {
      ...
      const sc = employee.manualMetrics.reduce(
        (n, x) => n + (fullMonthKeys.has(x.month.toISOString().slice(0, 7)) ? Number(x.scAchieved || 0) : 0),
        0,
      );
      ...
      return {
        ...
        totalRechargePercent: totalRechargeTarget
          ? Number(((totalRecharge / totalRechargeTarget) * 100).toFixed(1))
          : 0,
    ...
    console.error(error);
    const e = apiError(error, "Failed to load C2C summary.");
