...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    for (const group of recordGroups) {
      const eid = employeeOf.get(group.retailerId);
      if (!eid) continue;
    ...
    const rows = employees.map((employee) => {
      ...
      return {
        ...
        transactionCount: perf.transactions,
        c2sAmount: perf.amount,
        lsoTarget,
        lsoAchieved: perf.lso,
    ...
    console.error(error);
    const e = apiError(error, "Failed to load C2S summary.");
