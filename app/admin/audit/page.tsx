...
export default async function Page({searchParams}:{searchParams:Promise<{q?:string;module?:string;action?:string}>}){
 ...
 const [rows,total,today,logins]=await Promise.all([
  ...
  prisma.auditLog.count({where:{action:"LOGIN",createdAt:{gte:new Date(new Date().setHours(0,0,0,0))}}})
 ]);
 ...
 return <main className="page admin-audit"><div className="admin-kicker">SECURITY & HISTORY</div><h1>Activity Log</h1><p className="permissions-sub">Recent login, account, permission and administrative activity.</p>
 ...
 <div className="card audit-list">{rows.length?rows.map(x=><div className="audit-row" key={x.id}><div className="audit-dot"/><div className="audit-main"><div><strong>{x.actorName}</strong><span>{x.actorRole}</span></div><p>{x.action.replaceAll("_"," ")}{x.targetName?<> · <b>{x.targetName}</b></>:null}</p><small>{x.detail||x.module}</small></div><div className="audit-meta"><b>{x.module}</b><span>{fmt(x.createdAt)}</span></div></div>):<div className="audit-empty">No matching activity found.</div>}</div></main>
}