...
export default async function Page({searchParams}:{searchParams:Promise<{q?:string;module?:string;action?:string}>}){
 ...
 <LiveFilterForm className="card audit-filters live-filter-v63"><input name="q" defaultValue={q} placeholder="Search user, target or activity" autoComplete="off"/><select name="module" defaultValue={module}><option value="">All modules</option>{modules.map(x=><option key={x.module}>{x.module}</option>)}</select><select name="action" defaultValue={action}><option value="">All actions</option>{actions.map(x=><option key={x.action}>{x.action}</option>)}</select><span className="live-filter-status"><Icon name="filter"/>Live filter</span>{(q||module||action)&&<Link className="btn btn-ghost" href="/admin/audit">Clear</Link>}</LiveFilterForm>
