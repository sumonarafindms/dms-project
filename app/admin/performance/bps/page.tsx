...
export default async function Page({searchParams}:{searchParams:Promise<{q?:string;month?:string;from?:string;to?:string}>}){
 const u=await requireUser(["ADMIN"]),s=await searchParams,month=normalizeMonth(s.from?.slice(0,7)||s.month),data=await listBpAssignments(u,month,s.q,s.from,s.to);
