...
export default async function Page({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{month?:string;from?:string;to?:string}>}){
 const u=await requireUser(["ADMIN"]),{id}=await params,s=await searchParams,month=normalizeMonth(s.from?.slice(0,7)||s.month),d=await bpAssignmentDetail(u,id,month,undefined,s.from,s.to);if(!d)notFound();const a=d.assignment;
