...
export default async function Page({searchParams}:{searchParams:Promise<{q?:string;month?:string;from?:string;to?:string}>}){
 await requireUser(["ADMIN"]);const s=await searchParams,q=(s.q||"").trim().toLowerCase(),month=normalizeMonth(s.from?.slice(0,7)||s.month),rows=await employeePerformance(`${month}-01`,undefined,s.from,s.to),{start,end}=monthBounds(`${month}-01`);
 const bp=await prisma.bpAssignment.findMany({where:{startDate:{lt:end},OR:[{endDate:null},{endDate:{gte:start}}]},include:{employee:{select:{supervisorId:true}},monthlyTargets:{where:{month:start},take:1}}});
 ...
 for(const b of bp){if(b.employee.supervisorId&&map.has(b.employee.supervisorId)){const x=map.get(b.employee.supervisorId)!;x.bps.add(b.retailerId);x.gaT+=(b.monthlyTargets[0]?.gaTarget??b.gaTarget)}}
