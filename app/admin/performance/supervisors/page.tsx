...
export default async function Page({searchParams}:{searchParams:Promise<{q?:string;month?:string}>}){
 await requireUser(["ADMIN"]);const s=await searchParams,q=(s.q||"").trim().toLowerCase(),month=normalizeMonth(s.month),rows=await employeePerformance(`${month}-01`),{start,end}=monthBounds(`${month}-01`);
 const bp=await prisma.bpAssignment.findMany({where:{startDate:{lt:end},OR:[{endDate:null},{endDate:{gte:start}}]},select:{employee:{select:{supervisorId:true}},gaTarget:true,retailerId:true}});
 ...
 for(const b of bp){if(b.employee.supervisorId&&map.has(b.employee.supervisorId)){const x=map.get(b.employee.supervisorId)!;x.bps.add(b.retailerId);x.gaT+=b.gaTarget}}
 ...
 return <main className="page admin-performance"><PerfHead title="Supervisor Performance" subtitle="Team-level target, achievement, RSO and BP overview." month={month} q={s.q||""} placeholder="Search supervisor"/>
