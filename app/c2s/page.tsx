...
export default function C2sPage(){
  ...
  async function load(overrides?:{month?:string;date?:string;from?:string;to?:string}){const nextMonth=overrides?.month||month,nextDate=overrides?.date||date,nextFrom=overrides?.from||fromDate,nextTo=overrides?.to||toDate;const p=new URLSearchParams({month:`${nextMonth}-01`,date:nextDate,from:nextFrom,to:nextTo,_:String(Date.now())}); const res=await fetch(`/api/c2s/summary?${p}`,{cache:"no-store"}); const data=await res.json(); if(!res.ok)return setMessage(data.error||"Failed to load C2S data"); setRows(data.rows||[]);setDailyRows(data.dailyRows||[]);setHistory(data.importHistory||[])}
  useEffect(()=>{load()},[month,date,fromDate,toDate]);
  async function upload(e:FormEvent<HTMLFormElement>){e.preventDefault();const input=e.currentTarget.elements.namedItem("file") as HTMLInputElement;if(!input.files?.[0])return;setLoading(true);setMessage("Reading month-to-date C2S report and rebuilding date-wise retailer sales...");const body=new FormData();body.append("file",input.files[0]);const res=await fetch("/api/import/C2S",{method:"POST",body});const data=await res.json();setLoading(false);if(!res.ok)return setMessage(data.error||"C2S import failed");if(data.duplicate)setMessage("This exact C2S file was already imported. Nothing was counted twice.");else{setMessage(`C2S updated ${data.reportStartDate} → ${data.reportEndDate}. ${data.successRows} retailers mapped, ${data.failedRows} failed, ${data.dailyRecordsStored} non-zero retailer/day sales stored.${data.assignmentWarnings?` ${data.assignmentWarnings} RSO assignment mismatch warning(s).`:""}`);if(data.reportStartDate&&data.reportEndDate){const nextMonth=data.reportEndDate.slice(0,7);setDate(data.reportEndDate);setMonth(nextMonth);setFromDate(data.reportStartDate);setToDate(data.reportEndDate);input.value="";await load({month:nextMonth,date:data.reportEndDate,from:data.reportStartDate,to:data.reportEndDate});return}}input.value="";await load()}
  ...
  return <main className="page ops-premium-page">
    ...
    <OpsFreshness label="C2S" businessDate={history[0]?.businessDate} uploadedAt={history[0]?.uploadedAt} fileName={history[0]?.fileName} range={`${fromDate} → ${toDate}`}/>

    {canAdd&&<OpsUpload title="Upload C2S File" subtitle="Import the cumulative ITop Up Sales workbook." sample="/api/samples/c2s" message={message} rule={<><b>LSO rule:</b> A retailer completes LSO when monthly C2S reaches at least 500 with at least 7 transactions. Each upload is the authoritative month-to-date snapshot: DMS replaces that month before saving the new file, then recalculates retailer sales and LSO from the refreshed data.</>}>
