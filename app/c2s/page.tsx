...
export default function C2sPage(){
  ...
  async function load(){const p=new URLSearchParams({month:`${month}-01`,date,from:fromDate,to:toDate}); const res=await fetch(`/api/c2s/summary?${p}`,{cache:"no-store"}); const data=await res.json(); if(!res.ok)return setMessage(data.error||"Failed to load C2S data"); setRows(data.rows||[]);setDailyRows(data.dailyRows||[]);setHistory(data.importHistory||[])}
